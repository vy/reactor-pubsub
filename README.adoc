...
https://travis-ci.org/vy/reactor-pubsub[image:https://secure.travis-ci.org/vy/reactor-pubsub.svg[Build Status]]
...
.20+|`pubsub.client` |`type=timer` .6+|`publish` latency
|`operation=publish`
|`projectName=<projectName>`
|`topicName=<topicName>`
|`result=<success\|failure>`
|`[rootCauseClassName=<rootCauseClassName>]`
|`type=timer` .6+|`pull` and `ack` latency
|`operation=<pull\|ack>`
|`projectName=<projectName>`
|`subscriptionName=<subscriptionName>`
|`result=<success\|failure>`
|`[rootCauseClassName=<rootCauseClassName>]`
|`type=counter` .4+|``publish``ed message count
|`operation=publish`
|`projectName=<projectName>`
|`topicName=<topicName>`
|`type=counter` .4+|``pull``ed and ``ack``ed message count
|`operation=<pull\|ack>`
|`projectName=<projectName>`
|`subscriptionName=<subscriptionName>`

.2+|`pubsub.stagedRateLimiter.permitWaitPeriod` |`type=summary` .2+|
|`name=<name>`
|============================

There are a couple of details that need further elaboration here:

- `PubsubPuller#pullAll()` repeatedly executes `pull` requests such that if the
  previous `pull` has returned an empty response, the next `pull` gets delayed
  by `PubsubPullerConfig#pullPeriod`. Hence the published
  `pubsub.client{type=timer,operation=pull}` metrics are a combination of both
  the full and the empty responses.
